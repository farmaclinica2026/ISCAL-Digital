...
const DoseItem = ({ label, value, isHighlighted, weight }: { label: string, value: string, isHighlighted: boolean, weight: string }) => {
  const calculatedDoses = useMemo(() => {
    ...
    // Regex to match patterns like "15-20mg/kg" or "5mg/kg" or "0,7-1mg/kg" or "25.000 UI/kg"
    const regex = /(\d+(?:[\.,]\d+)?(?:\.\d{3})?)\s*(?:-\s*(\d+(?:[\.,]\d+)?(?:\.\d{3})?))?\s*(m?g\/kg|UI\/kg)/gi;
    const matches = [...value.matchAll(regex)];
    if (matches.length === 0) return null;

    const parseValue = (val: string) => {
      if (!val) return 0;
      // If it has a dot followed by 3 digits and NO comma, it's likely thousands (e.g. 25.000)
      // If it has a comma, the comma is decimal.
      let clean = val;
    ...
    return matches.map(match => {
      const minVal = parseValue(match[1]);
      const maxVal = match[2] ? parseValue(match[2]) : null;
      const unit = match[3].toLowerCase();
      const isUI = unit.includes('ui');
      const baseUnit = isUI ? 'UI' : (unit.includes('mg') ? 'mg' : 'g');
      ...
      const formatNum = (num: number) => {
        if (isUI) {
          return `${num.toLocaleString('pt-BR', { maximumFractionDigits: 0 })} UI`;
        }
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm border border-green-100 p-1">
            <img src="/pwa-192x192.svg" alt="Logo" className="w-full h-full object-contain" referrerPolicy="no-referrer" />
          </div>
          <div>
            <h2 className="text-sm font-bold text-green-700 uppercase leading-tight">Guia Rápido para ajuste de antimicrobianos</h2>
                ...
                <div className="p-5 space-y-6">
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                       <DoseItem label="ClCr 30-50mL/min" value={item.egfr30_50} isHighlighted={highlightedCategory === 'crcl30_50'} weight={weight} />
                       <DoseItem label="ClCr < 30 mL/min sem hemodiálise" value={item.egfr_30} isHighlighted={highlightedCategory === 'crcl_30'} weight={weight} />
